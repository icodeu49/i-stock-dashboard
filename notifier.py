...
def run_daily_scan():
    ...
    for ticker in WATCHLIST:
        ...
        for timeframe_key, timeframe_label in intervals.items():
            ...
            try:
                # 📥 Step 1: Download matching interval for SPY reference baseline
                spy_df = yf.download("SPY", period="1y", interval=timeframe_key, progress=False, timeout=15, multi_level_index=False)
                if isinstance(spy_df.columns, pd.MultiIndex):
                    spy_df.columns = spy_df.columns.get_level_values(0)
                
                # 📥 Step 2: Download matching interval for target stock
                df = yf.download(ticker, period="1y", interval=timeframe_key, progress=False, timeout=15, threads=False, multi_level_index=False)
                if df.empty: 
                    continue
                ...
                # 🧮 Step 3: Crunch technical calculations matrix
