...
def load_memory():
    if os.path.exists(MEMORY_FILE) and os.path.getsize(MEMORY_FILE) > 0:
        try:
            ...
            print("Warning: agent_memory.json was corrupted. Resetting.")import os
import json
import smtplib
from email.mime.text import MIMEText
from email.mime.multipart import MIMEMultipart
...
TOPICS_AND_FEEDS = {
    "AI & Tech": "[https://news.google.com/rss/search?q=Artificial+Intelligence](https://news.google.com/rss/search?q=Artificial+Intelligence)",
    "Global Markets": "[https://news.google.com/rss/search?q=Stock+Market+Economy](https://news.google.com/rss/search?q=Stock+Market+Economy)",
    "Macroeconomics": "[https://news.google.com/rss/search?q=Macroeconomics](https://news.google.com/rss/search?q=Macroeconomics)",
    "AI Power": "[https://news.google.com/rss/search?q=AI+Power+energy](https://news.google.com/rss/search?q=AI+Power+energy)",
    "AI memory HBM companies": "[https://news.google.com/rss/search?q=AI+MEMORY+HBM](https://news.google.com/rss/search?q=AI+MEMORY+HBM)",
}
...
def send_email(html_content, link_map):
    ...
    # Dynamically inject pristine source links back into the code block safely
